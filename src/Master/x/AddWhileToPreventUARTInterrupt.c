    ...
    while (UARTCharsAvail(UART3_BASE)){
        ...
        if(Index_of_Rece_V_C_SetV > 15){
            ...
            for(i = 0; i < 5; i++){//是怕像上次一样清除不了中断导致数据不准确，找一个数保存得到的数据，即使再进入，元数据也得到了保存
                Copy_Real_Voltage += (int)(Received_Voltage_Current_Set_Value[i] - '0') * pow(10, 4 - i);   //18715
                Real_Voltage_Array[i] = Received_Voltage_Current_Set_Value[i];

                Copy_Current += (int)(Received_Voltage_Current_Set_Value[i + 5] - '0') * pow(10, 4 - i);   //09159
                Current_Array[i] = Received_Voltage_Current_Set_Value[i + 5];

                Copy_Set_Voltage += (int)(Received_Voltage_Current_Set_Value[i + 10] - '0') * pow(10, 4 - i);   //18715
