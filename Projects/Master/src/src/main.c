...
#define Sample_Resistor 0.1 //TODO: Tell me the value of resistor
//接收数据帧
...
//电流
volatile unsigned int Current = 0;  //Voltage on Sample_Resistor
unsigned char Current_Array[5];
float I = 0;//Current on Sample_Resistor
...
void InitPWM_D1_D0(void)
{
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOD);    //D0
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOC);    //c4
    ...
    //PWMGenConfigure(PWM0_BASE, PWM_GEN_6, PWM_GEN_MODE_DOWN | PWM_GEN_MODE_NO_SYNC);

    PWMDeadBandEnable(PWM0_BASE, PWM_GEN_3, SysCtlClockGet() / 50e3 * DEADZONETIME, SysCtlClockGet() / 50e3 * DEADZONETIME);
    //PWMDeadBandEnable(PWM0_BASE, PWM_GEN_6, SysCtlClockGet() / 50e3 * DEADZONETIME, SysCtlClockGet() / 50e3 * DEADZONETIME);
...
void PWM_ParaMeter_Period_Set_D1_D0(unsigned long Period)
{
    ...
    //PWMGenPeriodSet(PWM0_BASE, PWM_GEN_6, Period);
}
void PWM_ParaMeter_Duty_Set_D1_D0(unsigned long Period, float duty)
{

    PWMPulseWidthSet(PWM0_BASE, PWM_OUT_6, Period * duty);
    //PWMPulseWidthSet(PWM1_BASE, PWM_OUT_1, Period * duty);

}

void TurnOnPWM_C4_D0(void)
{
...
/*******************************PID结构体*******************************/
void Monitor_If_Current_Over_2_Ampere(void){
    while(1){
        unsigned int status = UARTIntStatus(UART3_BASE, true);
        UARTIntClear(UART3_BASE, status);
        I = Current / Sample_Resistor;
        if(I >= 2.00){
            TurnOffPWM_C4_D0();
...
void main(void)
{
    ...
    while (1){
        ...
        //TODO:想一下数据接受逻辑是否正确
        Monitor_If_Current_Over_2_Ampere();
    }
